...
const binaryName =
  process.platform === "win32" ? "cloudstate.exe" : "cloudstate";
...
async function downloadBinaryFromNpm(version) {
  const url = `https://registry.npmjs.org/${platformSpecificPackageName}/-/${platformSpecificPackageName}-${version}.tgz`;
...
async function fetchLatestVersion() {
  const response = await fetch(
    `https://registry.npmjs.org/${platformSpecificPackageName}/latest`
  );
